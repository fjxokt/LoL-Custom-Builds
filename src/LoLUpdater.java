...
public class LoLUpdater {
    ...
    final static String updateUrl = "http://lcb.spreadeas.com/update.txt";
    final static String changelistUrl = "http://lcb.spreadeas.com/changelist";
    ...
    public static int compareVersions(String v1, String v2) {
	 ...
	 public static void checkUpdate(boolean silent) {    	
    	 ...
    	 int newVersion = compareVersions(v, LoLWin.version);
    	 // if more recent
    	 if (newVersion == 1) {	 
    		 int res = JOptionPane.showConfirmDialog(null, getInfoPanel(v), LocaleString.string("New version available"), JOptionPane.YES_NO_OPTION, JOptionPane.INFORMATION_MESSAGE);
    	 ...
    	 // no update
    		 ...
    		 if (!silent) {
    			 JOptionPane.showMessageDialog(null, LocaleString.string("Version $0$ up to date", LoLWin.version),
    					 LocaleString.string("Update"), JOptionPane.INFORMATION_MESSAGE);
    		 }
    	 }
     }
	 ...
	 public static JPanel getInfoPanel(String v) {
		 ...
		 try {
    		 ...
    		 while ((line = buff.readLine()) != null) {
    			 if (line.startsWith("[" + v)) {
    				 news = true;
    	    		 buf.append("<b><u>What is new between versions " + LoLWin.version + " and " + v + " ?</u></b><br/>");
    			 }
    			 if (news) {
    				 if (line.length() == 0) {
    					 continue;
    				 }
    				 if (line.startsWith("[" + LoLWin.version)) {
    					 break;
    				 }
		 ...
		 pan.add(new JLabel(LocaleString.string("New version $0$ available", v)), BorderLayout.NORTH);
		 if (news) {
