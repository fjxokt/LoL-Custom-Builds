...
public class LoLSynchronizer extends JDialog implements ActionListener {
	...
	public LoLSynchronizer(LoLWin win, IniFile conf) {
       ...
       // pa.setAlignmentX(LEFT_ALIGNMENT);
		...
		proving = new JCheckBox("Proving Grounds");
	...
	public void synchronize() {
		if (classic.isSelected()) {
			synchronizeMode(GameMode.Classic);
		}
				
		if (dominion.isSelected()) {
			synchronizeMode(GameMode.Dominion);
		}
		
		if (proving.isSelected()) {
			synchronizeMode(GameMode.Proving_Grounds);
		}
	...
	private void synchronizeMode(GameMode mode) {
		load(mode);
	...
	private void save(GameMode mode) {
		// TODO: fix
		String fres = null;//mode.getFile();
	...
	private void load(GameMode mode) {
		// TODO: fix
		String fres = null;//mode.getFile();
	...
	private void synch() {
		...
		while (iter.hasNext()) {
			...
			// normalement tous les dossiers sont cr�es par LoLResources, a voir...
